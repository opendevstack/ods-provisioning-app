...
public class ConfluenceAdapter implements ICollaborationAdapter
{
    ...
    private static final String BLUEPRINT_PATTERN = "%s%s/create-dialog/1.0/space-blueprint/dialog/web-items";
    ...
    private String getBluePrintId(String projectTypeKey)
            throws IOException
    {
        ...
        for (Object obj : blueprints)
        {
            ...
            logger.debug("Blueprint: {} searchKey: {}",
                    blueprint.getBlueprintModuleCompleteKey(),
                    template);
            if (blueprint.getBlueprintModuleCompleteKey()
                    .equals(template))
            {
    ...
    int updateSpacePermissions(OpenProjectData data) throws IOException
    {
        ...
        Resource[] permissionFiles = pmrl
                .getResources(confluencePermissionFilePattern);

        int updatedPermissions = 0;

        logger.debug("Found permission sets: {}",
        ...
        template.put(PROJECT_TEMPLATE.TEMPLATE_KEY,
                (projectTypeKey != null
                        ...
                        && environment.containsProperty(
                                confluencetemplateKeyPrefix
                                        + projectTypeKey)
                        && projectTemplateKeyNames
                                .contains(projectTypeKey))
