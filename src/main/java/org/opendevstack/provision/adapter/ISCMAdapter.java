...
public interface ISCMAdapter extends IServiceAdapter {
  ...
  public enum URL_TYPE {
    URL_CLONE_HTTP, URL_CLONE_SSH, URL_BROWSE_HTTP
  }

  /**
   * Create an SCM project / container for various component repositories created later thru
   * {@link #createAuxiliaryRepositoriesForODSProject(OpenProjectData, String, String[])} and
   * {@link #createComponentRepositoriesForODSProject(OpenProjectData, String)} <b>Special
   * Attention: </b> {@link OpenProjectData#specialPermissionSet} may be true, hence the implementor
   * needs to take care about setting accurate permissions based on
   ...
   *        name of the component can be found as option in the map with key being
   *        {@link OpenProjectData#COMPONENT_ID_KEY}
   * @return a Map with key being the key for the repo and a map with the repo links
