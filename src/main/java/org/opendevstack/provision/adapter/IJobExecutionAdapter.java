...
public interface IJobExecutionAdapter extends IServiceAdapter {
  ...
  /**
   * Create platform projects, e.g. openshift projects to house later components created thru
   * {@link #provisionComponentsBasedOnQuickstarters(OpenProjectData)}
   * 
   ...
   * @return the project with filled {@link OpenProjectData#platformDevEnvironmentUrl},
   *         {@link OpenProjectData#platformTestEnvironmentUrl},
   *         {{@link OpenProjectData#platformBuildEngineUrl} and
   *         {@link OpenProjectData#lastExecutionJobs} which contains the link the jobs that were
   *         kicked off
