...
/**
 * Abstraction to handle the OkHTTP restClient and use it in the same way from different classes to
...
public class OldRestClient {
  private static final Logger logger = LoggerFactory.getLogger(OldRestClient.class);
  ...
  public void getSessionId(String url) throws IOException {
    try {
      callHttpInternal(url, null, false, HttpVerb.HEAD, null, null);
    } catch (HttpException httpX) {
      if (RETRY_HTTP_CODES.contains(httpX.getResponseCode())) {
        callHttpInternal(url, null, true, HttpVerb.HEAD, null, null);
      } else {
        throw httpX;
      }
    }
  ...
  public <T> T callHttp(
      String url, Object input, boolean directAuth, HttpVerb verb, Class<T> returnType)
  ...
  public <T> T callHttpTypeRef(
      String url, Object input, boolean directAuth, HttpVerb verb, TypeReference<T> returnType)
      throws HttpException, IOException {
    try {
  ...
  public void callHttpBasicFormAuthenticate(String url, String username, String password)
      throws IOException {
    RequestBody body =
