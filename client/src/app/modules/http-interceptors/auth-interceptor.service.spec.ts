...
xdescribe(`AuthHttpInterceptor`, () => {
  ...
  it('should add an Authorization header', inject([HttpTestingController, ProjectService], (httpMock: HttpTestingController, projectService: ProjectService)  => {
    projectService.getProjectById('ASAP').subscribe(response => {
      expect(response).toBeTruthy();
    });

    const httpRequest = httpMock.expectOne({url: '/mock/url/ASAP', method: 'GET'});
    httpMock.verify();
